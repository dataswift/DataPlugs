  ...
  def status: Action[AnyContent] = tokenUserAuthenticatedAction.async { implicit request =>
    request.headers.get("X-Auth-Token").map { token =>
      ...
      Try(SignedJWT.parse(token)).map { parsedToken =>
        ...
        Future.successful(Forbidden(Json.toJson(ErrorMessage("Forbidden", s"Token could not get parsed. Token is: $token"))))
      }
    }.getOrElse {
      Future.successful(InternalServerError(Json.toJson(ErrorMessage("Internal Service Error", s"Could not find token in headers"))))
    }
  }
