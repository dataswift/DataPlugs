 ...
 */
@Singleton
class OAuth2InfoDAOImpl @Inject() (protected val dbConfigProvider: DatabaseConfigProvider)
  extends DelegableAuthInfoDAO[OAuth2Info] with HasDatabaseConfigProvider[SlickPostgresDriver] {
  ...
  def add(loginInfo: LoginInfo, authInfo: OAuth2Info): Future[OAuth2Info] = {
    val q = Tables.UserOauth2Info += toDbModel(loginInfo, authInfo)
  ...
  def update(loginInfo: LoginInfo, authInfo: OAuth2Info): Future[OAuth2Info] = {
    val q = for {
      auth <- Tables.UserOauth2Info if auth.providerId === loginInfo.providerID && auth.userId === loginInfo.providerKey
    } yield (auth.accessToken, auth.tokenType, auth.expiresIn, auth.refreshToken, auth.params)

    val action = q.update((authInfo.accessToken, authInfo.tokenType, authInfo.expiresIn, authInfo.refreshToken,
      authInfo.params.map(m => Json.toJson(m))))
  ...
  def find(loginInfo: LoginInfo): Future[Option[OAuth2Info]] = {
    val q = Tables.UserOauth2Info.filter(auth => auth.providerId === loginInfo.providerID && auth.userId === loginInfo.providerKey)
