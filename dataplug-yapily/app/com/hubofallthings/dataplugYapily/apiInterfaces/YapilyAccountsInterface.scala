...
class YapilyAccountsInterface @Inject() (
  ...
  val defaultApiEndpoint: ApiEndpointCall = YapilyAccountsInterface.defaultApiEndpoint
  ...
  override def validateMinDataStructure(rawData: JsValue): Try[JsArray] = {
    (rawData \ "data").toOption.map {
      case data: JsArray if data.validate[List[YapilyAccounts]].isSuccess =>
      ...
      case data =>
        logger.error(s"Error parsing JSON object: ${data.toString} ${data.validate[List[YapilyAccounts]]}")
...
object YapilyAccountsInterface {
